...
const (
	...
	transferCompleteUri = "/api/management/v1/deployments/artifacts/directupload/:id/complete"
)
...
func (c *Client) DirectUpload(
	...
	if rsp.StatusCode >= httpErrorBoundary {
		...
		_, err := client.DoPostRequest(token, strings.ReplaceAll(transferCompleteUri, ":id", id), c.client)
		if err != nil {
